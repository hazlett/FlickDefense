...
public class QT_ConvertMesh : EditorWindow
{
    ...
    static void Init()
    {
        ...
        window.maxSize = new Vector2(300, 275);
        ...
        for (int x = 0; x < MeshGOs.Length; x++)       
        {
            ...
            if (isSkinnedMesh[x])
            {
                ...
                if (SMRs[SMRIndex].sharedMesh.blendShapeCount > 0 && SMRs[SMRIndex].sharedMesh.boneWeights.Length == 0) //can't access blendshape mesh data in Unity 4.5, so we have to convert it to a simpleprop.
                {
                    ...
                    RenderSimpleProp(SMRs[SMRIndex], MeshGOs[x]);
                }
                else
                {
    ...
    private void RenderSimpleProp(SkinnedMeshRenderer SMR,GameObject currentGO) //for meshes with blendshapes. access is unsupported in unity 4.5
    {
        //make a copy of the source mesh
    ...
    private Material CreateMasterMaterial(string folderPath, string matName)
    {
        ...
        if(newMat==null)
        {           
            newMat = new Material(Shader.Find("QuantumTheory/VertexColors/IBL/Diffuse"));
         newMat.name = matName;
